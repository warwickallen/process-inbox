/* ProcessInbox.coffee [38bba1ca95be11649387fd294095c1e69cc5009c] 2018-09-27 16:13:30
...
var t_id, labels, notified, __, notification_text;
...
function currentDayRange(start_day, end_day) {
  var dayNumber;
  ...
  return function() {
    var retval, today;
    today = new Date().getDay();
    retval = start_day < end_day ? today >= start_day && today <= end_day : today <= start_day && today >= end_day;
    log(today + " is " + (retval ? "" : "not ") + "in the range (" + start_day + ", " + end_day + ").");
    return retval;
  };
};
...
function processMail(rules) {
  ...
  log("PROCESS MAIL [38bba1ca95be11649387fd294095c1e69cc5009c] 2018-09-27 16:13:30");
